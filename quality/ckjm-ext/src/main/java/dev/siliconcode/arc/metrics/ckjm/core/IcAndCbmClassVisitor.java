...
import dev.siliconcode.arc.metrics.ckjm.core.utils.FieldAccess;
import dev.siliconcode.arc.metrics.ckjm.core.utils.MethodCoupling;
import dev.siliconcode.arc.metrics.ckjm.core.utils.MethodInvokation;
...
public class IcAndCbmClassVisitor extends AbstractClassVisitor {
    ...
    /**
     * How many inherited methods use a field, that is defined
        ...
        for (MethodCoupling mc : mMethodCouplings) {
            if (mc.getClassA().equals(mClassName)) {
                ...
                if (mc.getClassB().equals(mClassName))
                    log.error("Both of the involved in MethodCoupling classes are the investigated class!", new RuntimeException());
            } else {
                coupledParents.add(mc.getClassA());
                if (!mc.getClassB().equals(mClassName))
                    log.error("None  of the involved in MethodCoupling classes is the investigated class!", new RuntimeException());
            }
        }
